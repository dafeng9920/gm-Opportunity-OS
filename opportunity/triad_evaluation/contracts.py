...
class RoleAssessmentRecord:
 role_assessment_id:str; role_id:str; candidate_id:str; input_asset_id:str; judge_assessment_id:str; runtime_source:JudgeRuntimeSource; assessment_result:str; provenance:tuple[str,...]; version:str
 def __post_init__(self):
  if not all((self.role_assessment_id,self.role_id,self.candidate_id,self.input_asset_id,self.judge_assessment_id,self.assessment_result,self.provenance,self.version)): raise ValueError('role assessment is legacy_unbound or incomplete')
...
@dataclass(frozen=True, slots=True)
class TriadDecisionArtifactBoundary: context_id:str; candidate_id:str; input_asset_id:str; version:str
